...
copyPassHistory:function(a){c.info("account:copyPassHistory");m.copyPassHistory(a)},copy:function(a){c.info("account:copy");f({r:[a.data("action-route"),a.data("item-id")]},"account")},saveFavorite:function(a,b){c.info("account:saveFavorite");var d="on"===a.data("status"),n=d?a.data("action-route-off"):a.data("action-route-on"),u=sysPassApp.requests.getRequestOpts();u.url=sysPassApp.util.getUrl(e.entrypoint,{r:n+"/"+a.data("item-id"),sk:sysPassApp.sk.get(),isAjax:1});sysPassApp.requests.getActionCall(u,function(c){sysPassApp.msg.out(c);
...
positive:{title:sysPassApp.config.LANG[43],onClick:function(a){a.preventDefault();"function"===typeof b&&b(c)}}})},getSelection:function(a){a=a.data("selection");var b=[];return a&&($(a).find(".is-selected").each(function(){b.push($(this).data("item-id"))}),0===b.length)?!1:b}};return{getContent:f,showFloatingBox:h,closeFloatingBox:p,appMgmt:v,account:m,accountManager:{restore:function(a){c.info("accountManager:restore");
...
"_blank")):(c.info("Downloading file"),fetch(a,{credentials:"same-origin"}).then(function(r){var d=r.headers.get("Content-Disposition");if(!r.ok||!d)throw 0;var n=/filename="?([^";]+)"?/.exec(d);return r.blob().then(function(bl){return{blob:bl,name:n?n[1]:"download"}})}).then(function(o){var u=URL.createObjectURL(o.blob),k=document.createElement("a");k.href=u;k.download=o.name;document.body.appendChild(k);k.click();k.remove();URL.revokeObjectURL(u);sysPassApp.msg.ok(sysPassApp.config.LANG[72])}).catch(function(){sysPassApp.msg.error(sysPassApp.config.LANG[1])}))},delete:function(a){c.info("file:delete");var b='<div id="alert"><p id="alert-text">'+sysPassApp.config.LANG[15]+"</p></div>";mdlDialog().show({text:b,negative:{title:sysPassApp.config.LANG[44],onClick:function(a){a.preventDefault();sysPassApp.msg.error(sysPassApp.config.LANG[44])}},positive:{title:sysPassApp.config.LANG[43],onClick:function(b){b=
...
onClick:function(){var d=sysPassApp.requests.getRequestOpts();d.url=sysPassApp.util.getUrl(e.entrypoint,{r:a.data("action-route")});d.data=a.serialize()+"&sk="+sysPassApp.sk.get();sysPassApp.requests.getActionCall(d,function(b){sysPassApp.msg.out(b);a.find(":input[type=password]").val("")})}}})},backup:function(a){c.info("config:backup");g.state.update(a);var b=sysPassApp.requests.getRequestOpts();b.url=e.entrypoint+
