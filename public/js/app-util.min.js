...
sysPass.Util=function(a){var d={config:{passLength:0,minPasswordLength:8,complexity:{chars:!0,numbers:!0,symbols:!0,uppercase:!0,numlength:12},charset:{special:"!\"#$%&'*+,./:;=?@\\^`|~[]{}()<>",number:"1234567890",char:"abcdefghijklmnopqrstuvwxyz"}},randomIndex:function(b){var c=new Uint32Array(1),f=4294967296-4294967296%b;do window.crypto.getRandomValues(c);while(c[0]>=f);return c[0]%b},random:function(b){a.info("password:random");var c="";this.config.complexity.symbols&&(c+=this.config.charset.special);this.config.complexity.numbers&&(c+=this.config.charset.number);this.config.complexity.chars&&(c+=this.config.charset.char,this.config.complexity.uppercase&&
(c+=this.config.charset.char.toUpperCase()));var f=function(){for(var a="",b=0;b++<d.config.complexity.numlength;)a+=c.charAt(d.randomIndex(c.length));return a},m=function(b){a.info("password:random:checkComplexity");b=b.split("");if(d.config.complexity.symbols){var c=b.some(function(a){return 0<d.config.charset.special.indexOf(a)});if(!1===c)return c}if(d.config.complexity.numbers&&(c=b.some(function(a){return 0<d.config.charset.number.indexOf(a)}),!1===c)||d.config.complexity.chars&&
(c=b.some(function(a){return 0<d.config.charset.char.indexOf(a)}),!1===c))return c;if(d.config.complexity.chars&&d.config.complexity.uppercase){var f=d.config.charset.char.toUpperCase();b=b.some(function(a){return 0<f.indexOf(a)});if(!1===b)return b}return!0},e="";for(var n=0;1E3>n;n++){e=f();if(m(e))break}this.config.passLength=e.length;"function"===typeof b&&b(e,zxcvbn(e))},output:function(b,c){a.info("password:outputResult");var g=$("#password-strength-"+c.attr("id"));c=$("#password-level-"+c.attr("id"));var f=b.score;c.removeClass("weak good strong strongest");g.removeClass("weak good strong strongest");var h=g.find(".password-strength-bar"),j=g.find(".password-strength-label");
if(0===this.config.passLength){c.attr("data-level-msg","");g.hide();h.css("width","0");j.text("")}else{g.show();if(this.config.passLength<this.config.minPasswordLength){c.attr("data-level-msg",sysPassApp.config.LANG[11]).addClass("weak");g.addClass("weak");h.css("width","15%");j.text(sysPassApp.config.LANG[11])}else if(0===f){c.attr("data-level-msg",sysPassApp.config.LANG[9]+" - "+b.feedback.warning).addClass("weak");g.addClass("weak");h.css("width","25%");j.text(sysPassApp.config.LANG[9])}else if(1===f||2===f){c.attr("data-level-msg",sysPassApp.config.LANG[8]+" - "+b.feedback.warning).addClass("good");g.addClass("good");h.css("width","50%");j.text(sysPassApp.config.LANG[8])}else if(3===f){c.attr("data-level-msg",sysPassApp.config.LANG[7]).addClass("strong");g.addClass("strong");h.css("width","75%");j.text(sysPassApp.config.LANG[7])}else if(4===f){c.attr("data-level-msg",sysPassApp.config.LANG[10]).addClass("strongest");g.addClass("strongest");h.css("width","100%");j.text(sysPassApp.config.LANG[10])}}},
