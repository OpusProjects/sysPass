...
sysPass.Theme=function(a){var e=$.Event("theme:update"),b={elems:{$wrap:$("#wrap-loading"),$loading:$("#loading")},show:function(d){void 0!==d&&!0===d&&b.elems.$wrap.addClass("overlay-full");b.elems.$wrap.show();b.elems.$loading.addClass("is-active")},hide:function(){b.elems.$wrap.removeClass("overlay-full").hide();b.elems.$loading.removeClass("is-active")},upgradeFull:function(){b.elems.$wrap.addClass("overlay-full")}},f=function(d,r){sysPassApp.util.password.random(function(c,h){d.attr("data-pass",
c);var a=d.parent(),b=r||$("#"+d.attr("id")+"_repeat");sysPassApp.util.password.output(h,d);h=new MaterialTextfield;a.find("input:password").val(c);a.addClass(h.CssClasses_.IS_DIRTY).removeClass(h.CssClasses_.IS_INVALID);0<b.length&&(b.attr("data-pass",c),b.val(c).parent().addClass(h.CssClasses_.IS_DIRTY).removeClass(h.CssClasses_.IS_INVALID),sysPassApp.encryptFormValue(b),b.trigger("change"))})},g=function(){var a='<div id="box-complexity">\n                <div>\n                    <label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="checkbox-chars">\n                        <input type="checkbox" id="checkbox-chars" class="mdl-checkbox__input" name="checkbox-chars" checked/>\n                        <span class="mdl-checkbox__label">'+
...
$(this);if("true"!==c.attr("data-pass-upgraded")){var a=sysPassApp.util.uniqueId(),d=c.parent(),b=c.closest("form");a=c.attr("id")+"-"+a;var l=b.find("#"+c.attr("id")+"_repeat");l.attr("id",a+"_repeat");l.data("match-source",c);c.attr("id",a);c.attr("data-pass",c.val());b='<button id="menu-password-'+a+'" class="mdl-button mdl-js-button mdl-button--icon" type="button" title="'+sysPassApp.config.LANG[27]+'"><i class="material-icons">more_vert</i></button>\n                <ul class="mdl-menu mdl-js-menu" for="menu-password-'+
a+'">\n                <li class="mdl-menu__item passGen"><i class="material-icons">settings</i>'+sysPassApp.config.LANG[28]+'</li>\n                <li class="mdl-menu__item passComplexity"><i class="material-icons">vpn_key</i>'+sysPassApp.config.LANG[29]+'</li>\n                <li class="mdl-menu__item reset"><i class="material-icons">refresh</i>'+sysPassApp.config.LANG[30]+"</li></ul>";d.after("<div class='password-strength' id='password-strength-"+a+"' style='display:none'><div class='password-strength-track'><div class='password-strength-bar'></div></div><span class='password-strength-label'></span></div>");d.after('<div class="password-actions" />');d.next(".password-actions").prepend("<i id='password-level-"+a+"' class=\"showpass material-icons clip-pass-field password-level\" data-clipboard-target='"+
a+"' data-level-msg= '' title=\""+sysPassApp.config.LANG[32]+'">remove_red_eye</i>').append(b);c.on("keyup",function(){sysPassApp.util.password.checkLevel(c);this.dataset.pass=c.val()});d=c.parent().next();d.find(".passGen").on("click",function(){f(c,l);c.blur()});d.find(".passComplexity").on("click",function(){g()});d.find(".showpass").on("mouseover",function(){""!==this.dataset.levelMsg?$(this).attr("title",this.dataset.levelMsg+"\n\n"+c[0].dataset.pass):$(this).attr("title",c[0].dataset.pass)});
d.find(".reset").on("click",function(){c.val("");c[0].dataset.pass="";0<l.length&&l.val("");componentHandler.upgradeDom()});c.attr("data-pass-upgraded","true");componentHandler.upgradeDom()}});a.find(".passwordfield__input-show").each(function(){var a=$(this),h=sysPassApp.util.uniqueId(),c=a.attr("id")+"-"+h,m=a.data("match-source");a.attr("data-pass",a.val());var d=$("<i id='password-level-"+c+"' class=\"showpass material-icons password-level\" data-level-msg='' title=\""+sysPassApp.config.LANG[32]+'">remove_red_eye</i>');a.attr("id",c);if(m){a.parent().after("<div class='password-match' id='password-match-"+c+"' style='display:none'><i class='material-icons password-match-icon'></i><span class='password-match-label'></span></div>")}else{a.parent().after("<div class='password-strength' id='password-strength-"+c+"' style='display:none'><div class='password-strength-track'><div class='password-strength-bar'></div></div><span class='password-strength-label'></span></div>")}a.parent().after('<div class="password-actions" />');var e=a.parent().next(".password-actions");if(1===a.data("clipboard")){var b=$('<i class="clip-pass-icon material-icons" title="'+sysPassApp.config.LANG[34]+'" data-clipboard-target="#'+a.attr("id")+'">content_paste</i>');e.prepend(d).prepend(b)}else e.prepend(d);if(m){var q=$("#password-match-"+c),u=q.find(".password-match-icon"),p=q.find(".password-match-label"),k=function(){var s=m[0].dataset.pass||m.val(),t=a[0].dataset.pass||a.val();if(""===t){q.hide();return}q.show();s===t?(q.removeClass("no-match").addClass("match"),u.text("check_circle"),p.text(sysPassApp.config.LANG[73])):(q.removeClass("match").addClass("no-match"),u.text("cancel"),p.text(sysPassApp.config.LANG[74]))};a.on("keyup",function(){this.dataset.pass=a.val();k()});a.on("change",k);m.on("keyup.match change.match",k)}else{a.on("keyup",function(){sysPassApp.util.password.checkLevel(a);this.dataset.pass=a.val()})}d.on("mouseover",function(){d.attr("title",a[0].dataset.pass)})})},n=function(d){a.info("setupDatePicker");var c={format:"YYYY-MM-DD",lang:sysPassApp.config.BROWSER.LOCALE.substr(0,2),time:!1,cancelText:sysPassApp.config.LANG[44],okText:sysPassApp.config.LANG[43],clearText:sysPassApp.config.LANG[30],nowText:sysPassApp.config.LANG[56],minDate:new Date,triggerEvent:"dateIconClick"};d.find(".password-datefield__input").each(function(){var a=
$(this),d=a.parent();a.bootstrapMaterialDatePicker(c);var b=d.find("input[name="+a.data("dst-unix")+"]");""!==a.val()&&b.val(moment.tz(a.val(),sysPassApp.config.BROWSER.TIMEZONE).format("X"));d.next("i").on("click",function(){a.trigger("dateIconClick")});a.on("change",function(){b.val(moment.tz(a.val(),sysPassApp.config.BROWSER.TIMEZONE).format("X"))})})},m=function(){a.info("theme:update");componentHandler.upgradeDom();$("body").trigger(e)};return{passwordDetect:k,password:f,update:m,viewsTriggers:{main:function(){var a=
