...
d.find(".reset").on("click",function(){c.val("");c[0].dataset.pass="";0<l.length&&l.val("");componentHandler.upgradeDom()});c.attr("data-pass-upgraded","true");componentHandler.upgradeDom()}});a.find(".passwordfield__input-show").each(function(){var a=$(this),h=sysPassApp.util.uniqueId(),c=a.attr("id")+"-"+h;a.attr("data-pass",a.val());var d=$("<i id='password-level-"+c+"' class=\"showpass material-icons password-level\" data-level-msg='' title=\""+sysPassApp.config.LANG[32]+'">remove_red_eye</i>');a.attr("id",c);a.parent().after("<div class='password-strength' id='password-strength-"+c+"' style='display:none'><div class='password-strength-track'><div class='password-strength-bar'></div></div><span class='password-strength-label'></span></div>");a.parent().after('<div class="password-actions" />');var e=a.parent().next(".password-actions");if(1===a.data("clipboard")){var b=$('<i class="clip-pass-icon material-icons" title="'+sysPassApp.config.LANG[34]+'" data-clipboard-target="#'+
a.attr("id")+'">content_paste</i>');e.prepend(d).prepend(b)}else e.prepend(d);a.on("keyup",function(){sysPassApp.util.password.checkLevel(a);this.dataset.pass=a.val()});d.on("mouseover",function(){d.attr("title",a[0].dataset.pass)})})},n=function(d){a.info("setupDatePicker");var c={format:"YYYY-MM-DD",lang:sysPassApp.config.BROWSER.LOCALE.substr(0,2),time:!1,cancelText:sysPassApp.config.LANG[44],okText:sysPassApp.config.LANG[43],clearText:sysPassApp.config.LANG[30],nowText:sysPassApp.config.LANG[56],minDate:new Date,triggerEvent:"dateIconClick"};d.find(".password-datefield__input").each(function(){var a=
