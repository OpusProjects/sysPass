...
use SP\Application\Notification\Services\Mail;
...
final class CoreDefinitions
{
    ...
    private static function getPaths(string $rootPath): array
    {
        ...
        $varPath = FileSystem::buildPath($rootPath, 'var');
        $resourcesPath = FileSystem::buildPath($rootPath, 'resources');
        $configPath = getFromEnv('CONFIG_PATH', FileSystem::buildPath($rootPath, 'config'));
        $logFile = getFromEnv('LOG_FILE', FileSystem::buildPath($configPath, 'syspass.log'));

        return [
            // Path::APP is the persistent application-state dir used as the backup source;
            // after dissolving app/ that is the config dir.
            [Path::APP, $configPath],
            [Path::SQL, $sqlPath],
            ...
            [Path::MODULES, FileSystem::buildPath($rootPath, 'src', 'Infrastructure', 'Adapter', 'In')],
            [Path::LOCALES, FileSystem::buildPath($resourcesPath, 'locales')],
            [Path::BACKUP, FileSystem::buildPath($varPath, 'backup')],
            [Path::CACHE, FileSystem::buildPath($varPath, 'cache')],
            [Path::TMP, FileSystem::buildPath($varPath, 'temp')],
