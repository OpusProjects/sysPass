...
namespace SP\Tests\Unit\Infrastructure\Adapter\In\Web\Controllers\ConfigGeneral;
...
use SP\Infrastructure\Adapter\In\Web\Controllers\ConfigGeneral\DownloadConfigBackupController;
use SP\Infrastructure\Adapter\In\Web\Controllers\ConfigGeneral\DownloadLogController;
use SP\Infrastructure\Adapter\In\Web\Controllers\ConfigGeneral\SaveController;
 ...
 *
 * `SaveController::saveAction()` is the exception: it delegates to `ConfigTrait::saveConfig()`,
 * which does carry a `catch` — around `ConfigFileService::save()` — so that one also gets a test for
 * the failure path. The other two actions here (both file downloads) have no `catch` of their own;
 * whatever they throw propagates past the controller.
 *
 * `applicationForASignedInUser()` cannot be reused as-is: `SimpleControllerBase::$eventDispatcher`
 * is typed to the concrete, `final` `EventDispatcher`, not `EventDispatcherInterface`, and that
 * assignment runs before `initialize()` ever gets to check the ACL — so a stub of the interface is
 * a property-type `TypeError` on every one of these controllers, refusal or not.
...
class RefusalsTest extends WebControllerTestCase
{
     ...
     */
    #[Test]
    public function downloadingTheConfigBackupIsRefusedWhenTheAclDenies(): void
    {
        $configBackupService = $this->createMock(ConfigBackupService::class);
        $configBackupService->expects(self::never())->method('getBackup');
        ...
        new DownloadConfigBackupController(
            $application,
            $this->simpleControllerHelper($acl, 'configGeneral', 'downloadConfigBackup'),
            $configBackupService
        );
    }
     ...
     */
    #[Test]
    public function downloadingTheLogIsRefusedWhenTheAclDenies(): void
    {
        $application = $this->signedInUserApplication();
        $acl = $this->aclThatRefuses();
        ...
        new DownloadLogController(
            $application,
            $this->simpleControllerHelper($acl, 'configGeneral', 'downloadLog'),
            new PathsContext()
        );
    }
    ...
    {
        $appLock = $this->createMock(AppLockHandler::class);
        $appLock->expects(self::never())->method('lock');
        $appLock->expects(self::never())->method('unlock');
    ...
    {
        $appLock = $this->createStub(AppLockHandler::class);
