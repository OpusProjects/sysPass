...
namespace SP\Tests\Unit\Infrastructure\Adapter\In\Web\Controllers\ConfigImport;
...
use SP\Infrastructure\Adapter\In\Web\Controllers\ConfigImport\ImportController;
 ...
 *
 * `ConfigImport` has a single controller, and it — like every other controller in the `Config*`
 ...
 * before the injected `ImportService` is ever reached. Reaching it needs a `SimpleControllerHelper`
 ...
 *
 * `importAction()` carries no `catch` of its own — whatever `ImportService::doImport()` (or the
 * `getFile()`/`FileHandler` construction ahead of it) throws propagates past the controller — so
 * there is no catch-arm test here, only the refusal.
 */
#[Group('unitary')]
class RefusalsTest extends WebControllerTestCase
{
    ...
    public function importingIsRefusedWhenTheAclDenies(): void
    {
        ...
        $importService->expects(self::never())->method('doImport');
        ...
        new ImportController(
            $application,
            $this->simpleControllerHelper($acl, 'configImport', 'import'),
            $importService
        );
    }
