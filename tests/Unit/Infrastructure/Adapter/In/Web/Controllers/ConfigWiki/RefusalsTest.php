...
use SP\Infrastructure\Adapter\In\Web\Controllers\ConfigWiki\SaveController;
 ...
 *
 * `ConfigWiki` has a single controller, and it — like every other controller in the `Config*`
 ...
 * `ConfigFileService::save()` — but unlike `ConfigAuth`/`ConfigEvents`, it is not reached
 * unconditionally: with wiki disabled in both the request and the stored config, the action answers
 * "No changes" before ever calling `saveConfig()`. Reaching the catch arm needs the stored config to
 * already have wiki enabled, so the "disable" branch runs and falls through to the save.
 */
#[Group('unitary')]
class RefusalsTest extends WebControllerTestCase
{
     ...
     * already has wiki enabled (and the request carries no `wiki_enabled` flag, so the stub's `bool`
     ...
     * fails, to reach `ConfigTrait::saveConfig()`'s catch arm. The config data also reports wiki as
    ...
    private function applicationWhoseConfigSaveThrows(): Application
    {
        ...
        $configData->method('isWikiEnabled')->willReturn(true);
