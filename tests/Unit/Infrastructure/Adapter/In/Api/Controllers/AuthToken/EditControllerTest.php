...
use SP\Infrastructure\Adapter\In\Api\Controllers\AuthToken\EditController;
...
class EditControllerTest extends UnitaryTestCase
{
    ...
    {
        $this->apiService->expects($this->once())->method('setup')->with(AclActionsInterface::AUTHTOKEN_EDIT);
        ...
        $this->authTokenService->expects($this->once())->method('update')->with($this->isInstanceOf(AuthToken::class));

        $response = $this->controller->editAction();
        $result = $response->getResponse();
        $this->assertEquals(0, $result['resultCode']);
        $this->assertEquals(5, $result['itemId']);
        $this->assertEquals('Authorization updated', $result['resultMessage']);
    }
     ...
     * the hash recomputed at edit time never match the raw value checked at use
     * time (SP\Application\Api\Services\Api::getMasterPassFromVault).
     */
    public function testEditActionWithSpecialCharacterPasswordIsReadRaw(): void
    {
