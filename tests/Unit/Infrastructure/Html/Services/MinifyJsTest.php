...
class MinifyJsTest extends UnitaryTestCase
{
    ...
    private function buildCheckWithEtag(int $numFiles = 1): array
    {
        ...
        $files = array_map(function () use ($hash) {
            ...
            $filePath = REAL_APP_ROOT . '/public/js/app-main.min.js';
        ...
        $this->request->expects(self::exactly(4))
                      ...
                      ->with(
                          new Callback(function (string $header) {
                              return $header === \SP\Infrastructure\Http\Header::IF_NONE_MATCH->value
                                     || $header === \SP\Infrastructure\Http\Header::CACHE_CONTROL->value
                                     || $header === \SP\Infrastructure\Http\Header::PRAGMA->value;
                          })
                      )
                      ->willReturn($etag);
    ...
    private function buildCheckWithoutEtag(int $numFiles = 1): array
    {
        ...
        $this->request->expects(self::once())
                      ->method('getHeader')
                      ->with(\SP\Infrastructure\Http\Header::IF_NONE_MATCH->value)
        ...
        $this->response->expects(self::exactly(5))
                       ...
                       ->with(
                           ...
                           [\SP\Infrastructure\Http\Header::ETAG->value, $etag],
                           [
                               \SP\Infrastructure\Http\Header::CACHE_CONTROL->value,
                               'public, max-age=2592000, must-revalidate'
                           ],
                           [\SP\Infrastructure\Http\Header::PRAGMA->value, 'public; maxage=2592000'],
                           [\SP\Infrastructure\Http\Header::EXPIRES->value, self::anything()],
                           [\SP\Infrastructure\Http\Header::CONTENT_TYPE->value, 'application/javascript; charset=UTF-8']
                       )
                       );
